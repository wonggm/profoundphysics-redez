---
title: Is Time a Vector?
category: mathematics
date: 2024-09-04
original_url: https://profoundphysics.com/is-time-a-vector/
excerpt: Why time has magnitude but not direction — and what special relativity forces us to give up.
canonical: false
---

# Is Time Actually A Vector or A Scalar? The Physics Explained

> Why time has magnitude but not direction — and what special relativity forces us to give up.

*Originally published at [profoundphysics.com](https://profoundphysics.com/is-time-a-vector/).*
...
### How Newtonian Physics Explains Time
...
## Why Time Is Actually a Coordinate (In Special Relativity)
...
Everything I’m going to talk about special relativity here is explained in much more detail in my [introduction to special relativity](https://profoundphysics.com/special-relativity-for-dummies-an-intuitive-introduction/).
...
As a sidenote; some people like to think of position in spacetime as a *four-dimensional position vector* (with a time and three space components), which would make time a **vector component**.  
  
This, however, is not really correct, because “spacetime position” itself is not a well-defined concept (at least, not a well-defined *vector*) in general relativity, but spacetime displacements are. So, you could technically define a **time interval** as a vector component, but **time itself is only a coordinate**, nothing else.
...
Essentially, this formulation is based on a field of mathematics called **geometric algebra**. Geometric algebra, on the other hand, is based on the mathematics of **multivectors** and **geometric products**.
...
The basic idea behind multivectors and bivectors goes as follows:

- A multivector can be thought of as a sort of generalization of regular vectors, and is a combination of scalars, vectors, bivectors, trivectors etc., meaning that it is a combination of **higher-dimensional vectors**.
...
- This idea can then be extended even further. **A bivector is a 2-dimensional vector object**, a trivector a 3-dimensional vector object and so forth.
- Now, regular vectors can be built out of a **basis** (the x,y-basis in this case):

$$V=V_x\overline{\text{x}}+V_y\overline{\text{y}}$$

The Vx and Vy here represent magnitudes in each direction and the x and y with little lines above them are the basis vectors in the x and y directions.
...
$$B=B_{xy}\overline{\text{x}}\overline{\text{y}}$$

Bxy is the magnitude of this bivector and the xy with a line above it is a basis bivector.
...
- When it comes to the direction of abivector, it has **two directional values** (degrees of freedom), its **orientation in space** as well as a kind of **rotational direction** that depends on the vectors it is built out of.

Usually, the basis vectors are represented more generally with the letter e, for example:

$$\overline{\text{x}}=e_1\\\overline{\text{y}}=e_2$$

Now, the interesting thing about bivectors is that a **unit bivector actually squares to -1** (just by how we choose to define it). For example, the square of a unit bivector e1e2 is:

$$\left(e_1e_2\right)^2=-1$$

But how is this useful for special relativity? The answer is that it actually provides a useful alternative for describing an **interval in spacetime**. The formula for a spacetime interval (S2) is:
...
The mathematics of this is not too difficult and you can find more about it in [this paper](https://arxiv.org/pdf/1205.5195.pdf).
